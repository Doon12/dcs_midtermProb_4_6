...
public class ShopImpl extends UnicastRemoteObject implements Shop {
    ...
    @Override
    public OrderInfo usrBuy(String usrName, String usrAdr, String type, String proID, int amount) throws RemoteException, InterruptedException {
        /*
        user submit order
         */
        ...
        if (goodList.get(proID) != null) {
            ReadWriteLock rwlock = goodList.get(proID).rwlock;
            ...
            if (goodList.get(proID).amount >= amount) { // check product amount redundant or not
                ...
                // Random sleep for lost update
                int product_amt = goodList.get(proID).amount;

                Thread.sleep((long)(Math.random()*1000));

                // when submit order is completed, edit product amount.
                //if (goodList.get(proID).amount == amount)
                if(product_amt == amount)
                    this.delProInfo(type, proID);
                else
                    this.editProInfo(type, proID, goodList.get(proID).price, product_amt-amount);
